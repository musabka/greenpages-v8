...
import { useAgentSettlementDetails, useConfirmAgentSettlement, useCancelAgentSettlement } from '@/lib/hooks/useFinancial';

type SettlementStatus = 'DRAFT' | 'PENDING_AGENT' | 'PENDING_MANAGER' | 'PENDING_ADMIN' | 'CONFIRMED' | 'CANCELLED';
...
const statusConfig: Record<SettlementStatus, { label: string; color: string; icon: any }> = {
  ...
  PENDING_AGENT: { label: 'بانتظار المندوب', color: 'bg-yellow-100 text-yellow-700', icon: Clock },
  PENDING_MANAGER: { label: 'بانتظار تأكيدك', color: 'bg-blue-100 text-blue-700', icon: Clock },
  PENDING_ADMIN: { label: 'بانتظار الإدارة', color: 'bg-indigo-100 text-indigo-700', icon: Clock },
...
export default function SettlementDetailsPage() {
  ...
  const handleCancel = () => {
    ...
    if (reason) {
      cancelMutation.mutate(settlementId);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-green-600 font-mono font-bold mt-1">{settlement.settlementNumber}</p>
        </div>
      ...
      {/* Action Buttons */}
      {settlement.status === 'PENDING_MANAGER' && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
          <p className="text-yellow-800 mb-3">هذه التسوية بانتظار تأكيدك. يرجى مراجعة التفاصيل واتخاذ الإجراء المناسب.</p>
          <div className="flex gap-3">
            ...
            >
              ...
              {confirmMutation.isPending ? 'جاري التأكيد...' : 'تأكيد التسوية'}
            </button>
            <button
            ...
            >
              ...
              {cancelMutation.isPending ? 'جاري الإلغاء...' : 'إلغاء التسوية'}
            </button>
          </div>
        </div>
      )}
      ...
      <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-xl p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white/60 rounded-lg p-4">
            <p className="text-sm text-green-700">إجمالي مقبوضات المندوب</p>
            <p className="text-xl font-bold text-green-900">
              {Number(settlement.totalCollected || 0).toLocaleString()} ل.س
            </p>
          </div>
          <div className="bg-white/60 rounded-lg p-4">
            <p className="text-sm text-green-700">النقد بحوزة المندوب</p>
            <p className="text-xl font-bold text-red-600">
              {Number(settlement.cashOnHand || 0).toLocaleString()} ل.س
            </p>
          </div>
          <div className="bg-white/60 rounded-lg p-4">
            <p className="text-sm text-green-700">المسلم سابقاً</p>
            <p className="text-xl font-bold text-gray-700">
              {Number(settlement.previouslyDelivered || 0).toLocaleString()} ل.س
            </p>
          </div>
          <div className="bg-white/60 rounded-lg p-4">
            <p className="text-sm text-green-700">العمولات المستحقة للمندوب</p>
            <p className="text-xl font-bold text-purple-700">
              {Number(settlement.totalCommissions || 0).toLocaleString()} ل.س
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-green-50 border border-green-200 rounded-lg p-5">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <p className="text-sm text-green-700">المبلغ المستلم من المندوب</p>
                <p className="text-2xl font-bold text-green-700">
                  {Number(settlement.amountDelivered || 0).toLocaleString()} ل.س
                </p>
              </div>
            </div>
            <p className="text-xs text-green-600 mt-2">
              عدد التحصيلات: {settlement.collectionsCount || 0}
            </p>
          </div>
          ...
          <div className="bg-purple-50 border border-purple-200 rounded-lg p-5">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <p className="text-sm text-purple-700">العمولات المدفوعة للمندوب</p>
                <p className="text-2xl font-bold text-purple-700">
                  {Number(settlement.commissionsPaid || 0).toLocaleString()} ل.س
                </p>
              </div>
            </div>
            <p className="text-xs text-purple-600 mt-2">
              عدد العمولات: {settlement.commissionsCount || 0}
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-blue-700">صافي المبلغ المستلم</p>
              <p className="text-xs text-blue-600">(المبلغ المستلم - العمولات المدفوعة)</p>
            </div>
            <p className="text-3xl font-bold text-blue-700">
              {(Number(settlement.amountDelivered || 0) - Number(settlement.commissionsPaid || 0)).toLocaleString()} ل.س
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {settlement.cancelledAt && (
            <div>
              <p className="text-sm text-gray-500">تاريخ الإلغاء</p>
              <p className="font-medium">
                {new Date(settlement.cancelledAt).toLocaleDateString('ar-SA', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
                  hour: '2-digit',
      ...
      {settlement.cancellationReason && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-6">
          <h2 className="font-bold text-red-900 mb-2">سبب الإلغاء</h2>
          <p className="text-red-700 whitespace-pre-line">{settlement.cancellationReason}</p>
        </div>
      )}
    </div>
  );
