...
export default function NewBusinessPage() {
  ...
  // Get agent profile to filter governorates
  ...
  const governorates = useMemo<Governorate[]>(() => {
    const allGovernorates = Array.isArray(governoratesResponse)
    ...
    // إذا كان لدينا معلومات المندوب، صفّي المحافظات فقط للمحافظات المخصصة له
    if (agentProfileData?.governorates && Array.isArray(agentProfileData.governorates)) {
      const allowedGovernorateIds = new Set(agentProfileData.governorates.map((g: any) => g.id));
      return allGovernorates.filter(gov => allowedGovernorateIds.has(gov.id));
    }
    ...
    try {
      ...
      if (selectedPackageId && result?.data?.id) {
        try {
          ...
          console.error('فشل تعيين الباقة:', error);
          // Continue anyway - business was created successfully
        }
      }
      ...
      console.error('خطأ في إنشاء النشاط التجاري:', error);
